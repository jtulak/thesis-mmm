...
The \Cref{tab:results:overview} offers an overview of how many issues did every
...
exact condition as can be seen on \Cref{lst:results:sectorsize} in
...
In version 4.7.0 inn mkfs-specific files, 4 issues were found. Whole
...
\Cref{lst:results:logagnoComparison}. Closer look on this line reveals that
...
be seen on \Cref{tab:results:coverity:levels}. The numbers in this table are
...
specific categories, as can be seen on \Cref{fig:results:coverity:defects}
...
This section analyses the results from local execution of Coverity on the
...
These two issues also illustrates the difference between the aggressivness
...
\TODO{usefullness of the online services}
